...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Scanner scannerLine = new Scanner(System.in); //nextLine
        ...
        while (stepCode != 0) {
            ...
            switch (stepCode) {
                ...
                case 4:
                    ...
                    if (cafe != null) {
                        menuCafe(cafe, categoriesService, subcategoriesService, productsService);
                    } else {
    ...
    public static void menuAdmin(UsersService usersService, CardsService cardsService, CategoriesService categoriesService, SubcategoriesService subcategoriesService, ProductsService productsService) {
        ...
        while (stepCode != 0) {
            System.out.println("1.SET DISCOUNT PERCENT\t 2.SET PERCENT FOR CAFE\t 3.SEE PERCENTS\t " +
                    "4.SEE BALANCE\t 5.GET INFO ABOUT USER\t 6.GET INFO ABOUT CAFE\t ");
            System.out.println("7.ADD CATEGORY\t 8.ADD SUBCATEGORY\t 0.EXIT");
            stepCode = scannerInt.nextInt();
            switch (stepCode) {
                case 1:
    ...
    public static int getInfoUser(UsersService usersService, CardsService cardsService) {
        ...
        for (Card card : cards) {
            System.out.println(++index + ". Card name: " + card.getName() + " Cara number: " +
                    card.getCardNumber() + " Balance: " + card.getCardBalance());
        }
        return 1;
    }
    ...
    public static int addCategory(CategoriesService categoriesService) {
        ...
        boolean isValid = categoriesService.check(categoryName);
        if (!isValid) {
            return 0;
        }
    ...
    public static int addSubcategory(CategoriesService categoriesService, SubcategoriesService subcategoriesService) {
        ...
        boolean isValid = subcategoriesService.check(subcategoryName);
        if (!isValid) {
            return -2;
        }
        UUID categoryID = category.getId();
        Subcategory subcategory = new Subcategory(subcategoryName, categoryID);
    ...
    public static User signUpUser(UsersService usersService) {
        ...
        System.out.println(Response.ENTER_PHONE_NUMBER);
        String phoneNumber = scannerStr.next();
        boolean isSuccess = usersService.check(phoneNumber);
        if (!isSuccess)
            return null;
        System.out.println(Response.CONFIRM_CODE);
        int code = usersService.sendSMS();
        ...
        while (stepCode != 0) {
            ...
            switch (stepCode) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    menuHistoryUser(user, historiesService);
                    break;
            }
        }
    }
    ...
    public static int menuOrder(User user, LocationsService locationsService, CardsService cardsService, CategoriesService categoriesService, SubcategoriesService subcategoriesService, ProductsService productsService, CafesService cafesService, HistoriesService historiesService) {
        ...
        for (Product product : products) {
            ...
            System.out.print(++index + "." + product.getName() + " by Cafe: " + product.getCafeName() + " Price: " +
        ...
        if (product.getProductAmount() < amount) {
            System.out.println("Product amount is less than " + amount);
            return 0;
        }
        ...
        double priceWithoutCommission = product.getPrice() * (1 - Account.getCafePercent() / 100);
        double priceWithDiscount = product.getPrice() * (1 - Account.getDiscountPercent() / 100);
        if (priceWithDiscount > card.getCardBalance()) {
            System.out.println(Response.AMOUNT_OVER);
            return 0;
        }

        Cafe cafe = cafesService.get(product.getCafeName());
        Transaction.pay(priceWithDiscount, card, amount);
        Transaction.receive(priceWithoutCommission, cafe, amount);
        Transaction.incomeToSystem( product.getPrice(), amount);

        product.setProductAmount((short) (product.getProductAmount() - amount));
        HistoryCafe historyCafe = new HistoryCafe(priceWithoutCommission, user.getId(), product.getName(), amount, location);
        HistoryUser historyUser = new HistoryUser(priceWithDiscount, location, user.getId(), cafe.getName(), product.getName(), card, amount);
    ...
    public static void menuHistoryUser(User user, HistoriesService historiesService) {
        List<HistoryUser> historyUsers = historiesService.getHistories(user);
        int index = 0;
        for (HistoryUser historyUser : historyUsers) {
            Card card = historyUser.getCard();
            System.out.println(++index + ". Card name: " + card.getName() + " Card number: " +
                    card.getCardNumber() + " Product name: " + historyUser.getProductName() + " Cafe name: " +
                    historyUser.getCafeName() + " Price: " + historyUser.getPrice() + " Amount: " +
                    historyUser.getAmount());
        }
    }
    ...
    public static Cafe signUpCafe(CafesService cafesService) {
        ...
        Cafe cafe = new Cafe(name, username, password);
        cafesService.add(cafe);
        return cafe;
    }

    public static void menuCafe(Cafe cafe, CategoriesService categoriesService, SubcategoriesService subcategoriesService, ProductsService productsService) {
        Scanner scannerInt = new Scanner(System.in);
        int stepCode = 1;
        while (stepCode != 0) {
            ...
            switch (stepCode) {
                case 1:
                    ...
                    break;
                case 2:
                    System.out.println("Balance: " + cafe.getCafeBalance());
                    break;
                case 3:
                    break;
                case 0:
        ...
        while (stepCode != 0) {
            ...
            for (Product product : products) {
                ...
                System.out.println(++index + ". Name: " + product.getName() + " Subcategory: " +
                        subcategoryName + " Amount: " + product.getProductAmount() + " Price: " +
                        product.getPrice());
            }
            ...
            switch (stepCode) {
                case 1:
                    ...
                    System.out.println("Enter amount");
                    short amount = scannerInt.nextShort();
                    Product newProduct = new Product(name, cafe.getName(), price, amount, subcategory.getId(), cafe.getId());
